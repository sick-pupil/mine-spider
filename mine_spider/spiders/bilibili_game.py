# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 19:30:26 2023
...
class BilibiliGameSpider(Spider):
    
    name: str = 'bilibili-game'
    allowed_domains: list = ['bilibili.com']
    start_url: str = 'https://www.bilibili.com/v/game'
    custom_settings: dict = {
        ...
        'ITEM_PIPELINES': {
            'mine_spider.pipelines.BilibiliGamePipeline': 500
        },
    ...
    def start_requests(self):
        yield Request(url = self.start_url,
            meta = {
                'playwright': True, 
                'playwright_context': 'bilibili-game', 
            ...
            callback = self.game_parse,
            errback = self.err_game_callback,
            dont_filter = True,
        )
    ...
    async def game_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('game', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('game', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        gameRankItem = BilibiliRankItem()
                        gameRankItem['batch_id'] = self.batch_id
                        gameRankItem['channel_name'] = self.channel_name
                        gameRankItem['rank_item_bv'] = rank_item_bv
                        gameRankItem['block_name'] = area_name
                        gameRankItem['rank_item_num'] = rank_item_order
                        gameRankItem['rank_item_href'] = rank_item_link
                        gameRankItem['rank_item_detail_title'] = rank_item_title
                        gameRankItem['rank_item_pubman'] = rank_video_card_up_name
                        gameRankItem['rank_item_time'] = rank_video_card_pubdate
                        gameRankItem['rank_item_play'] = rank_video_card_play
                        gameRankItem['rank_item_danmu'] = rank_video_card_danmu
                        gameRankItem['rank_item_star'] = rank_video_card_star
                        gameRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[gameRankItem['rank_item_bv']] = gameRankItem
        
        # 产生日志
        for bv, gameRankItem in self.result_by_dict.items():
            pass
            block_name = gameRankItem['block_name']
            rank_item_href = gameRankItem['rank_item_href']
            rank_item_bv = gameRankItem['rank_item_bv']
            
            yield Request(url = self.url_prefix + rank_item_href,
                meta = {
                    'playwright': True, 
                    'playwright_context': 'bilibili-game-video-{}'.format(rank_item_bv), 
                    'playwright_context_kwargs': {
                        'ignore_https_errors': True,
                    },
                    'playwright_page_goto_kwargs': {
                ...
                callback = self.game_video_parse,
    ...
    async def game_video_parse(self, response, rank_item_bv : str):
        ...
        try:
            ...
            gameRankItem = self.result_by_dict[rank_item_bv]
            gameRankItem['rank_item_video_detail'] = rank_item_video_detail
            
            #if gameRankItem['rank_item_video_detail']['video_detail_reply'] is None or gameRankItem['rank_item_video_detail']['video_detail_reply'] == '':
            #    await page.screenshot(path='/screenshot_{}_{}.png'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #    with open(file='/screenshot_{}_{}.html'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            #        f.write(await page.content())
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            gameRankItem = self.result_by_dict[video_bv]
            gameRankItem['rank_item_up_detail'] = bilibiliUpInfo
    ...
    async def err_game_callback(self, failure):
        self.logger.info('err_game_callback')
