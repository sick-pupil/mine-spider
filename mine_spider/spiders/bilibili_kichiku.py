...
class BilibiliKichikuSpider(Spider):
    
    name: str = 'bilibili-kichiku'
    allowed_domains: list = ['bilibili.com']
    start_url: str = 'https://www.bilibili.com/v/kichiku'
    custom_settings: dict = {
        ...
        'ITEM_PIPELINES': {
            'mine_spider.pipelines.BilibiliKichikuPipeline': 500
        },
    ...
    def start_requests(self):
        yield Request(url = self.start_url,
            meta = {
                'playwright': True, 
                'playwright_context': 'bilibili-kichiku', 
            ...
            callback = self.kichiku_parse,
            errback = self.err_kichiku_callback,
            dont_filter = True,
        )
    ...
    async def kichiku_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('kichiku', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('kichiku', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        kichikuRankItem = BilibiliRankItem()
                        kichikuRankItem['batch_id'] = self.batch_id
                        kichikuRankItem['channel_name'] = self.channel_name
                        kichikuRankItem['rank_item_bv'] = rank_item_bv
                        kichikuRankItem['block_name'] = area_name
                        kichikuRankItem['rank_item_num'] = rank_item_order
                        kichikuRankItem['rank_item_href'] = rank_item_link
                        kichikuRankItem['rank_item_detail_title'] = rank_item_title
                        kichikuRankItem['rank_item_pubman'] = rank_video_card_up_name
                        kichikuRankItem['rank_item_time'] = rank_video_card_pubdate
                        kichikuRankItem['rank_item_play'] = rank_video_card_play
                        kichikuRankItem['rank_item_danmu'] = rank_video_card_danmu
                        kichikuRankItem['rank_item_star'] = rank_video_card_star
                        kichikuRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[kichikuRankItem['rank_item_bv']] = kichikuRankItem
        
        # 产生日志
        for bv, kichikuRankItem in self.result_by_dict.items():
            pass
            block_name = kichikuRankItem['block_name']
            rank_item_href = kichikuRankItem['rank_item_href']
            rank_item_bv = kichikuRankItem['rank_item_bv']
            
            yield Request(url = self.url_prefix + rank_item_href,
                meta = {
                    'playwright': True, 
                    'playwright_context': 'bilibili-kichiku-video-{}'.format(rank_item_bv), 
                    'playwright_context_kwargs': {
                        'ignore_https_errors': True,
                    },
                    'playwright_page_goto_kwargs': {
                ...
                callback = self.kichiku_video_parse,
    ...
    async def kichiku_video_parse(self, response, rank_item_bv : str):
        ...
        try:
            ...
            kichikuRankItem = self.result_by_dict[rank_item_bv]
            kichikuRankItem['rank_item_video_detail'] = rank_item_video_detail
            
            #if kichikuRankItem['rank_item_video_detail']['video_detail_reply'] is None or kichikuRankItem['rank_item_video_detail']['video_detail_reply'] == '':
            #    await page.screenshot(path='/screenshot_{}_{}.png'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #    with open(file='/screenshot_{}_{}.html'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            #        f.write(await page.content())
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            kichikuRankItem = self.result_by_dict[video_bv]
            kichikuRankItem['rank_item_up_detail'] = bilibiliUpInfo
            
            await page.close()
            await page.context.close()
            
            yield kichikuRankItem
        else:
            yield Request(url = response.request.url,
                meta = {
                    'playwright': True, 
    ...
    async def err_kichiku_callback(self, failure):
        self.logger.info('err_kichiku_callback')
