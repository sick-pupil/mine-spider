...
class BilibiliMusicSpider(Spider):
    ...
    async def music_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('music', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('music', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        musicRankItem = BilibiliRankItem()
                        musicRankItem['batch_id'] = self.batch_id
                        musicRankItem['channel_name'] = self.channel_name
                        musicRankItem['rank_item_bv'] = rank_item_bv
                        musicRankItem['block_name'] = area_name
                        musicRankItem['rank_item_num'] = rank_item_order
                        musicRankItem['rank_item_href'] = rank_item_link
                        musicRankItem['rank_item_detail_title'] = rank_item_title
                        musicRankItem['rank_item_pubman'] = rank_video_card_up_name
                        musicRankItem['rank_item_time'] = rank_video_card_pubdate
                        musicRankItem['rank_item_play'] = rank_video_card_play
                        musicRankItem['rank_item_danmu'] = rank_video_card_danmu
                        musicRankItem['rank_item_star'] = rank_video_card_star
                        musicRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[musicRankItem['rank_item_bv']] = musicRankItem
        
        # 产生日志
        for bv, musicRankItem in self.result_by_dict.items():
            pass
            block_name = musicRankItem['block_name']
    ...
    async def music_video_parse(self, response, rank_item_bv : str):
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            musicRankItem = self.result_by_dict[video_bv]
            musicRankItem['rank_item_up_detail'] = bilibiliUpInfo
