# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 19:35:10 2023
...
class BilibiliEntSpider(Spider):
    
    name: str = 'bilibili-ent'
    allowed_domains: list = ['bilibili.com']
    start_url: str = 'https://www.bilibili.com/v/ent'
    custom_settings: dict = {
        ...
        'ITEM_PIPELINES': {
            'mine_spider.pipelines.BilibiliEntPipeline': 500
        },
    ...
    def start_requests(self):
        yield Request(url = self.start_url,
            meta = {
                'playwright': True, 
                'playwright_context': 'bilibili-ent', 
            ...
            callback = self.ent_parse,
            errback = self.err_ent_callback,
            dont_filter = True,
        )
    ...
    async def ent_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('ent', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('ent', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        entRankItem = BilibiliRankItem()
                        entRankItem['batch_id'] = self.batch_id
                        entRankItem['channel_name'] = self.channel_name
                        entRankItem['rank_item_bv'] = rank_item_bv
                        entRankItem['block_name'] = area_name
                        entRankItem['rank_item_num'] = rank_item_order
                        entRankItem['rank_item_href'] = rank_item_link
                        entRankItem['rank_item_detail_title'] = rank_item_title
                        entRankItem['rank_item_pubman'] = rank_video_card_up_name
                        entRankItem['rank_item_time'] = rank_video_card_pubdate
                        entRankItem['rank_item_play'] = rank_video_card_play
                        entRankItem['rank_item_danmu'] = rank_video_card_danmu
                        entRankItem['rank_item_star'] = rank_video_card_star
                        entRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[entRankItem['rank_item_bv']] = entRankItem
        
        # 产生日志
        for bv, entRankItem in self.result_by_dict.items():
            pass
            block_name = entRankItem['block_name']
            rank_item_href = entRankItem['rank_item_href']
            rank_item_bv = entRankItem['rank_item_bv']
            
            yield Request(url = self.url_prefix + rank_item_href,
                meta = {
                    'playwright': True, 
                    'playwright_context': 'bilibili-ent-video-{}'.format(rank_item_bv), 
                    'playwright_context_kwargs': {
                        'ignore_https_errors': True,
                    },
                    'playwright_page_goto_kwargs': {
    ...
    async def ent_video_parse(self, response, rank_item_bv : str):
        ...
        try:
            ...
            entRankItem = self.result_by_dict[rank_item_bv]
            entRankItem['rank_item_video_detail'] = rank_item_video_detail
            
            #if entRankItem['rank_item_video_detail']['video_detail_reply'] is None or entRankItem['rank_item_video_detail']['video_detail_reply'] == '':
            #    await page.screenshot(path='/screenshot_{}_{}.png'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #    with open(file='/screenshot_{}_{}.html'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            #        f.write(await page.content())
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            entRankItem = self.result_by_dict[video_bv]
            entRankItem['rank_item_up_detail'] = bilibiliUpInfo
    ...
    async def err_ent_callback(self, failure):
        self.logger.info('err_ent_callback')
