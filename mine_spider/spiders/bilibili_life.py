# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 19:40:52 2023
...
class BilibiliLifeSpider(Spider):
    
    name: str = 'bilibili-life'
    allowed_domains: list = ['bilibili.com']
    start_url: str = 'https://www.bilibili.com/v/life'
    custom_settings: dict = {
        ...
        'ITEM_PIPELINES': {
            'mine_spider.pipelines.BilibiliLifePipeline': 500
        },
    ...
    def start_requests(self):
        yield Request(url = self.start_url,
            meta = {
                'playwright': True, 
                'playwright_context': 'bilibili-life', 
            ...
            callback = self.life_parse,
            errback = self.err_life_callback,
            dont_filter = True,
        )
    ...
    async def life_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('life', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('life', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        lifeRankItem = BilibiliRankItem()
                        lifeRankItem['batch_id'] = self.batch_id
                        lifeRankItem['channel_name'] = self.channel_name
                        lifeRankItem['rank_item_bv'] = rank_item_bv
                        lifeRankItem['block_name'] = area_name
                        lifeRankItem['rank_item_num'] = rank_item_order
                        lifeRankItem['rank_item_href'] = rank_item_link
                        lifeRankItem['rank_item_detail_title'] = rank_item_title
                        lifeRankItem['rank_item_pubman'] = rank_video_card_up_name
                        lifeRankItem['rank_item_time'] = rank_video_card_pubdate
                        lifeRankItem['rank_item_play'] = rank_video_card_play
                        lifeRankItem['rank_item_danmu'] = rank_video_card_danmu
                        lifeRankItem['rank_item_star'] = rank_video_card_star
                        lifeRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[lifeRankItem['rank_item_bv']] = lifeRankItem
        
        # 产生日志
        for bv, lifeRankItem in self.result_by_dict.items():
            pass
            block_name = lifeRankItem['block_name']
            rank_item_href = lifeRankItem['rank_item_href']
            rank_item_bv = lifeRankItem['rank_item_bv']
            ...
            yield Request(url = self.url_prefix + rank_item_href,
                ...
                callback = self.life_video_parse,
    ...
    async def life_video_parse(self, response, rank_item_bv : str):
        ...
        try:
            ...
            lifeRankItem = self.result_by_dict[rank_item_bv]
            lifeRankItem['rank_item_video_detail'] = rank_item_video_detail
            
            #if lifeRankItem['rank_item_video_detail']['video_detail_reply'] is None or lifeRankItem['rank_item_video_detail']['video_detail_reply'] == '':
            #    await page.screenshot(path='/screenshot_{}_{}.png'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #    with open(file='/screenshot_{}_{}.html'.format(rank_item_bv, datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            #        f.write(await page.content())
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            lifeRankItem = self.result_by_dict[video_bv]
            lifeRankItem['rank_item_up_detail'] = bilibiliUpInfo
    ...
    async def err_life_callback(self, failure):
        self.logger.info('err_life_callback')
