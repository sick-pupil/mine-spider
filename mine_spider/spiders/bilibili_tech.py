# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 19:36:45 2023
...
class BilibiliTechSpider(Spider):
    
    name: str = 'bilibili-tech'
    allowed_domains: list = ['bilibili.com']
    start_url: str = 'https://www.bilibili.com/v/tech'
    custom_settings: dict = {
        ...
        'ITEM_PIPELINES': {
            'mine_spider.pipelines.BilibiliTechPipeline': 500
        },
    ...
    def start_requests(self):
        yield Request(url = self.start_url,
            meta = {
                'playwright': True, 
                'playwright_context': 'bilibili-tech', 
            ...
            callback = self.tech_parse,
            errback = self.err_tech_callback,
            dont_filter = True,
        )
    ...
    async def tech_parse(self, response):        
        ...
        # 可以正常访问
            ...
            #await page.screenshot(path='/screenshot_{}_{}.png'.format('tech', datetime.now().strftime("%Y%m%d%H%M%S")), full_page=True)
            #with open(file='/screenshot_{}_{}.html'.format('tech', datetime.now().strftime("%Y%m%d%H%M%S")), mode='w', encoding='utf-8') as f:
            ...
            for area_grid in bili_area_grid_list:
                ...
                for rank_item_index in range(len(rank_item_list)):
                    if rank_item_index < 9:
                        ...
                        techRankItem = BilibiliRankItem()
                        techRankItem['batch_id'] = self.batch_id
                        techRankItem['channel_name'] = self.channel_name
                        techRankItem['rank_item_bv'] = rank_item_bv
                        techRankItem['block_name'] = area_name
                        techRankItem['rank_item_num'] = rank_item_order
                        techRankItem['rank_item_href'] = rank_item_link
                        techRankItem['rank_item_detail_title'] = rank_item_title
                        techRankItem['rank_item_pubman'] = rank_video_card_up_name
                        techRankItem['rank_item_time'] = rank_video_card_pubdate
                        techRankItem['rank_item_play'] = rank_video_card_play
                        techRankItem['rank_item_danmu'] = rank_video_card_danmu
                        techRankItem['rank_item_star'] = rank_video_card_star
                        techRankItem['rank_item_coin'] = rank_video_card_coin
                        
                        self.result_by_dict[techRankItem['rank_item_bv']] = techRankItem
        
        # 产生日志
        for bv, techRankItem in self.result_by_dict.items():
            pass
            block_name = techRankItem['block_name']
            rank_item_href = techRankItem['rank_item_href']
            rank_item_bv = techRankItem['rank_item_bv']
    ...
    async def up_info_parse(self, response, video_bv : str, up_link_id : str):
        ...
        if await page.locator("//div[@class='h-basic']").count() != 0:
            ...
            techRankItem = self.result_by_dict[video_bv]
            techRankItem['rank_item_up_detail'] = bilibiliUpInfo
    ...
    async def err_tech_callback(self, failure):
        self.logger.info('err_tech_callback')
